...
describe("partenaireDossierBuckets", () => {
  it("classe un dossier planifié comme nouveau", () => {
    ...
    expect(isDossierPlanifie(dossier)).toBeTrue();
    expect(isDossierEnCours(dossier)).toBeFalse();
  });

  it("classe un dossier sans date dans à planifier", () => {
    const dossier = {
      statut: "planifie",
      demarre_at: null,
      termine_at: null,
    ...
    expect(isDossierAPlanifier(dossier)).toBeTrue();
    expect(isDossierPlanifie(dossier)).toBeFalse();
  });
  ...
  it("répartit la liste en 4 sections organisationnelles", () => {
    const buckets = splitDossiersByLifecycle([
      { id: "a", statut: "planifie", statut_facturation: "a_facturer", date_a_confirmer: true },
      { id: "e", statut: "planifie", statut_facturation: "a_facturer", date_a_confirmer: false },
    ...
    expect(buckets.aPlanifier.map((d) => d.id)).toEqual(["a"]);
    expect(buckets.planifies.map((d) => d.id)).toEqual(["e"]);
