...
export const getDemandeClient = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase
      .from("demande_requests")
      .select(
        "id, nom, email, telephone, code_postal, type_bien, puissance, type_installation, distance_m, abonnement_kva, type_compteur, phase, notes, formule, type_demande, nb_bornes, status",
      )
      .eq("id", data.id)
      .maybeSingle();
    if (error) throw new Error(error.message);
