...
function PublicAttachment() {
  ...
  {annule ? <div className="print:hidden rounded-md border-2 border-destructive bg-destructive/10 p-4"><strong>Attachement annulé par IRVE Technologie</strong><p className="text-sm text-muted-foreground">Ce document n’est plus valable. Un nouvel attachement vous sera transmis si les travaux sont revalorisés.</p></div> : null}
  {!annule && a.statut === "facture" ? <div className="print:hidden rounded-md border border-primary/40 bg-primary/10 p-4"><strong>Attachement facturé</strong><p className="text-sm text-muted-foreground">La facture correspondante vous a été adressée.</p></div> : null}
  {!annule && majApresEnvoi ? <div className="print:hidden rounded-md border-2 border-neon-yellow/60 bg-neon-yellow/15 p-4"><strong className="text-neon-yellow">Document mis à jour le {new Date(a.updated_at).toLocaleString("fr-FR")}</strong><p className="text-sm text-muted-foreground">Vous consultez la version la plus récente : elle peut différer de l’e-mail que vous avez reçu.</p></div> : null}
  <p className="print:hidden text-xs text-muted-foreground">Statut : <strong>{({ brouillon: "En préparation", envoye: "Envoyé pour validation", propose: "Valorisation proposée", accepte: "Accepté", refuse: "Refusé", facture: "Facturé", annule: "Annulé" } as Record<string, string>)[a.statut] ?? a.statut}</strong>{a.sent_at ? ` · Envoyé le ${new Date(a.sent_at).toLocaleDateString("fr-FR")}` : ""}{a.updated_at ? ` · Dernière mise à jour le ${new Date(a.updated_at).toLocaleString("fr-FR")}` : ""}</p>
  {answered && <div className={`print:hidden rounded-md border-2 p-4 ${a.statut === "refuse" ? "border-destructive/60 bg-destructive/15" : "border-primary/60 bg-primary/15"}`}><strong className={a.statut === "refuse" ? "text-destructive" : "text-primary"}>{a.statut === "refuse" ? "Attachement refusé" : "Attachement accepté"}</strong>{a.signataire_nom ? <p className="text-sm text-muted-foreground">Réponse de {a.signataire_nom}</p> : null}</div>}
  {pending && <div className="print:hidden rounded-md border-2 border-neon-cyan/60 bg-neon-cyan/15 p-4"><strong className="text-neon-cyan">Valorisation proposée — en attente de validation par IRVE Technologie</strong><p className="text-sm text-muted-foreground">{euro(Number(proposition.total_ht))} HT proposés par {proposition.signataire_nom}.</p></div>}
  ...
  {mode ? <section className="print:hidden rounded-md border border-primary/40 bg-card p-5 space-y-4">
    ...
    {a.validation_requise && !answered && !annule && !pending ? <section className="print:hidden rounded-md border border-border bg-card p-5 space-y-4"><h2 className="font-bold">Valider cet attachement</h2><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nom et prénom du signataire" /><div className="flex flex-wrap gap-3"><Button disabled={name.trim().length < 2 || reply.isPending} onClick={() => reply.mutate("accepte")}><CheckCircle2 /> Accepter l’attachement</Button><Button variant="outline" disabled={name.trim().length < 2 || reply.isPending} onClick={() => reply.mutate("refuse")}><XCircle /> Refuser</Button></div>{error && <p className="text-sm text-destructive">{error}</p>}</section> : null}
